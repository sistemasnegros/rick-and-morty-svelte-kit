...
<script lang="ts">
	...
	import Card from '../../components/cards/episode-card.component.svelte';
	...
	let episodes = RickAndMortyServiceIns.findEpisode({ page: 1 }).then((result) => result);
	let searchFields = {};
	let currentPage = 1;

	const submit = async (fields: any) => {
		episodes = await RickAndMortyServiceIns.findEpisode(fields);
		searchFields = fields;
		currentPage = 1;
	};

	const numberPageOnClick = async (page: number) => {
		episodes = await RickAndMortyServiceIns.findEpisode({ page, ...searchFields });
		currentPage = page;
	};
</script>
...
{#await episodes}
	<Spinner />
{:then episodes}
	<Paginator metadata={episodes.info} {numberPageOnClick} currentPageProps={currentPage} />
	<div class="episodes-body">
		{#each episodes.results as episode}
			<Card {episode} />
		{/each}
	</div>
{:catch error}
